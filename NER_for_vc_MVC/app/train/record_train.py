...
def create_table(project_id, dataset_id, table_id_write):
    ...
    schema = [
        ...
        bigquery.SchemaField("Model Result", "STRING", mode="REQUIRED"),
...
def add_record_begin(dataset_name, dataset_size):
    ...
    query = f"""
        INSERT INTO `intern-project-415606.NER_web`.training_record (UUID, Status, `Dataset Description`, `Job Created Time`, `Job Ended Time`, `Job Duration`, `Model Path`, `Model Result`, `Result Path`)
        VALUES ('{job_uuid}', 'In Progress', '{dataset_name} - {dataset_size}', '{start_time}', '{start_time}', 0, '-', '-', '-')
    """
    try:
        client.query(query)
    except Exception as e:
...
def add_record_done(job_uuid, model_path, model_result, result_path):
    credentials = set_credential()
    client = bigquery.Client(credentials=credentials)
    end_time = datetime.now(pytz.timezone('Asia/Taipei')).strftime('%Y-%m-%d %H:%M:%S')
    query = f"""
        UPDATE `intern-project-415606.NER_web`.training_record
        SET `Status` = 'Completed', `Job Ended Time` = '{end_time}', `Job Duration` = TIMESTAMP_DIFF(TIMESTAMP('{end_time}'), TIMESTAMP(`Job Created Time`), SECOND), `Model Path` = '{model_path}', `Model Result` = '{model_result}', `Result Path` = '{result_path}'
        WHERE UUID = '{job_uuid}'
    """
    try:
        client.query(query)
